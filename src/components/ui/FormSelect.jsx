...
function FormSelect({ name, control, options, disabled = false, onValueChange, className = '' }) {
  ...
  return (
    <div ref={rootRef} className={`relative ${className}`}>
      ...
      <div className={`absolute z-50 mt-2 w-full origin-top overflow-hidden rounded-[16px] border border-[#e5e5e5] bg-white p-1.5 shadow-[0_18px_50px_rgba(0,0,0,0.14)] transition duration-150 ${open ? 'visible translate-y-0 scale-100 opacity-100' : 'invisible -translate-y-1 scale-[0.98] opacity-0'}`}>
        <div role="listbox" aria-label={name} className="max-h-60 overflow-y-auto">
