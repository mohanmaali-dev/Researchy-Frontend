...
import { FiArchive, FiBookOpen, FiDatabase, FiFileText, FiHelpCircle, FiHome, FiLogOut, FiMenu, FiMessageCircle, FiX } from 'react-icons/fi'
...
function LearningLayout() {
  ...
  const handleLogout = async () => {
    setLoggingOut(true)
    try { await logout(); navigate('/login', { replace: true }) } finally { setLoggingOut(false) }
  }
  ...
  return (
    <div className="min-h-screen bg-[#f2f2f1] p-2.5 text-ink sm:p-4">
      <header className="relative z-30 flex h-18 items-center rounded-[10px] bg-white px-4 sm:px-5">
        ...
        <div className="ml-auto flex items-center gap-2"><div className="hidden text-right sm:block"><p className="max-w-40 truncate text-xs font-medium text-[#333]">{user?.name || 'Workspace'}</p><p className="text-[10px] text-[#999]">Personal workspace</p></div><span className="grid size-10 place-items-center rounded-full bg-[#f5f5f5] text-sm font-medium text-[#555]">{(user?.name || 'W').trim().charAt(0).toUpperCase()}</span><button type="button" onClick={() => setSidebarOpen(true)} className="grid size-10 place-items-center rounded-full bg-[#f5f5f5] text-[#555] lg:hidden" aria-label="Open menu"><FiMenu aria-hidden="true" /></button></div>
      </header>
      ...
      <div className="mt-3 flex items-start gap-3">
        <aside className={`fixed inset-y-3 left-3 z-50 flex w-64 flex-col overflow-y-auto rounded-[10px] bg-white p-4 shadow-2xl transition-transform duration-300 lg:sticky lg:top-3 lg:h-[calc(100vh-6.75rem)] lg:translate-x-0 lg:shadow-none ${sidebarOpen ? 'translate-x-0' : '-translate-x-[115%]'}`}>
          ...
          <div className="mt-auto space-y-1 pt-6"><NavLink to="/learning/archived" className={linkClassName}><FiArchive aria-hidden="true" /> Archived Topics</NavLink><NavLink to="/learning/demo-data" className={linkClassName}><FiDatabase aria-hidden="true" /> Demo data</NavLink><button type="button" onClick={handleLogout} disabled={loggingOut} className="flex w-full items-center gap-3 rounded-md px-3.5 py-2.5 text-sm font-medium text-[#666] transition hover:bg-red-50 hover:text-red-600 disabled:opacity-50"><FiLogOut aria-hidden="true" /> {loggingOut ? 'Signing out...' : 'Sign out'}</button></div>
        </aside>
