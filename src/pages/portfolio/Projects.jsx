...
const desktopColumns = '5rem minmax(9rem,1fr) minmax(12rem,1.45fr) minmax(9rem,1fr) 5.75rem 5.75rem 6.5rem 7rem 7.5rem 9.5rem'
  ...
  return (
    <div className="space-y-3">
      ...
      <section className="rounded-lg bg-white p-4 sm:p-6">
        ...
        <div className="mt-4">
          {loading ? <TableLoadingState label="Loading projects" headers={['Image', 'Project name', 'Description', 'Technologies', 'Live site', 'GitHub', 'Status', 'Featured', 'Updated', '']} template={desktopColumns} minWidth="1360px" /> : error && !projects.length ? <ErrorState message={error} onRetry={loadProjects} backTo="/portfolio" backLabel="Portfolio overview" /> : !projects.length ? (
